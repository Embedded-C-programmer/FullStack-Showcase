...
const ChatArea = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    const handleIncomingCall = (callData) => {
      ...
      const validatedCallData = {
        ...callData,
        call: {
          ...callData.call,
          caller: callData.call.caller || callData.caller,
          type: callData.call.type || 'video'
        }
      };
      
      if (validatedCallData.call.conversationId === activeConversation?._id) {
        setIncomingCall(validatedCallData);
        toast((t) => (
          <div>
            <p>{validatedCallData.caller?.username || 'Someone'} is calling...</p>
            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem' }}>
              <button
                onClick={() => {
                  acceptCall(validatedCallData);
                  toast.dismiss(t.id);
                }}
                style={{
                  padding: '0.5rem 1rem',
          ...
          duration: 30000,
          icon: validatedCallData.call?.type === 'video' ? '📹' : '📞'
        });
      }
    };
  ...
  const initiateCall = (type) => {
    ...
    if (!otherUser) {
      toast.error('Cannot find other user');
      return;
    }
    
    // Set temporary call state immediately for UI feedback
    const tempCall = {
    ...
    socketService.once('call:initiated', ({ call, roomId }) => {
      // Ensure call object has required properties
      setActiveCall({ 
        ...call, 
        roomId,
        caller: call?.caller || user,
        type: call?.type || type,
        status: 'ringing'
      });
    });
  ...
  const acceptCall = (callData) => {
    if (!callData || !callData.roomId || !callData.call) {
