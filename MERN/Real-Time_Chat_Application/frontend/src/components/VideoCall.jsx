...
const VideoCall = ({ call, onEnd }) => {
  ...
  if (!call) {
    console.error('VideoCall: call prop is undefined');
    return null;
  }
  ...
  const [videoEnabled, setVideoEnabled] = useState(call?.type === 'video' || false);
  ...
  const initializeCall = async () => {
    try {
      ...
      if (!stream) {
        ...
        toast.error('Camera/microphone permission required. Please allow access in browser settings and try again.');
        onEnd();
        return;
      }
      ...
      >
        <div className="call-header">
          <div className="call-info">
            <span className="call-title">{call?.caller?.username || 'Unknown'}</span>
            ...
            {!videoEnabled && (
              <div className="video-placeholder">
                <div className="avatar-placeholder">
                  {call?.caller?.username?.[0]?.toUpperCase() || 'U'}
                </div>
              </div>
            )}
          </div>
